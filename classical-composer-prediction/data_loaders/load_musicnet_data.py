...
def load_musicnet_data(*args, **kwargs) -> pd.DataFrame:
    ...
    train_ids = [int(file.replace(".wav", "")) for file in os.listdir(os.path.join(data_dir, 'musicnet', 'train_data'))]
    test_ids = [int(file.replace(".wav", "")) for file in os.listdir(os.path.join(data_dir, 'musicnet', 'test_data'))]
    ...
    print(f"Number of training samples: {len(metadata_df[metadata_df['split'] == 'train'])}")
    print(f"Number of testing samples: {len(metadata_df[metadata_df['split'] == 'test'])}")
    return metadata_df
